...
def get_skin_detail(hash_name: str = Path(..., description="URL-encoded hash name")):
    ...
    price_map: dict[str, SkinPricePoint] = {}
    for h in hist:
        d = str(h["date"])
        price_map[d] = SkinPricePoint(
            date=d,
            price=float(h["median_price"]),
            predicted=None,
            predictedLSTM=None,
            volume=h["volume"],
        )
    ...
    for p in lstm_preds:
        d = str(p["target_date"])
        if d in price_map:
            price_map[d].predictedLSTM = float(p["predicted_price"])
        else:
            price_map[d] = SkinPricePoint(
                date=d, price=None,
                predicted=None,
                predictedLSTM=float(p["predicted_price"]), volume=0,
            )
